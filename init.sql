...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='contacts' AND column_name='preferred_contact_way') THEN
        ALTER TABLE contacts ADD COLUMN preferred_contact_way TEXT DEFAULT 'Mobil';
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='contacts' AND column_name='contacts') THEN
        ALTER TABLE contacts ADD COLUMN contacts JSONB;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='processes' AND column_name='storage_link') THEN
        ALTER TABLE processes ADD COLUMN storage_link TEXT;
    END IF;
...
-- Alter contacts.id to TEXT if needed (for new installs it's already TEXT)
-- Existing installs with SERIAL will keep working since TEXT accepts numeric strings
