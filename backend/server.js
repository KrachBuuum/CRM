...
async function runMigrations() {
  ...
  // Alter column types for old SERIAL/INTEGER -> TEXT
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (result.rowCount === 0) return res.status(401).json({ error: "Ungueltige Zugangsdaten" });
...
app.post("/api/notes", requireAuth, async (req, res) => {
  try {
    const n = req.body;
    const ts = n.timestamp || n.timestamp || new Date().toISOString();
    const r = await pool.query(
...
async function start() {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (i === maxRetries - 1) { console.error("[db] could not connect"); process.exit(1); }
